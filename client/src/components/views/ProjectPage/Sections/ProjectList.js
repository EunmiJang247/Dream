...
function ProjectList(props) {
  ...
  var now = dayjs()
  const remaindays = dayjs(props.project.duedate).diff(
    dayjs(now.format()),
    'days'
  )
  ...
  return (
    <>
      {props.MyPostProject && (
        ...
        <ListBody>
          <div style={{ width: '20%' }}>
            <ProjectPleft>{props.project.teamname}</ProjectPleft>
          </div>
          <div style={{ width: '45%' }}>
            <Link to={{ pathname: `/project/detail/${props.project._id}` }}>
              <ProjectPleft>
                [{props.project.servicecate}] {props.project.projectdesc}
              </ProjectPleft>
            </Link>
            <ProjectSubdesc>
          ...
          <div style={{ width: '10%' }}>
            <ProjectCenter>{props.project.meetingcycle}</ProjectCenter>
          </div>
          <div style={{ width: '10%' }}>
            <SeeApplierModal projectid={props.project._id} />
            {/* 지원자선택하는 부분 */}
            <Link to={{ pathname: `/project/modify/${projectId}` }}>
              <ProjectModifyButton>
                프로젝트
                <br />
                수정하기
      ...
      {!props.MyPostProject && (
        ...
        <ListBody>
          ...
          <div style={{ width: '10%' }}>
            ...
            <RegiDueP>
              ~{dayjs(props.project.duedate).format('YYYY-MM-DD')}
            </RegiDueP>
